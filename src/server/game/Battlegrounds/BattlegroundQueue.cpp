...
GroupQueueInfo *BattlegroundQueue::AddGroup(Player *leader, Group *grp, BattlegroundTypeId BgTypeId,
                                            ...
                                            uint32 arenateamid, uint32 PreviousOpponentsArenaTeamId) {
    ...
    if (isRated && sWorld->getBoolConfig(CONFIG_ARENA_QUEUE_ANNOUNCER_ENABLE)) {
        ...
        if (team)
            sWorld->SendWorldText(LANG_ARENA_QUEUE_ANNOUNCE_WORLD_JOIN, team->GetName().c_str(), ginfo->ArenaType,
                                  ginfo->ArenaType, ginfo->ArenaTeamRating);
    }
        ...
        if (!isRated && !isPremade && sWorld->getBoolConfig(CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_ENABLE)) {
            if (Battleground *bg = sBattlegroundMgr->GetBattlegroundTemplate(ginfo->BgTypeId)) {
                ...
                for (itr = m_QueuedGroups[bracketId][BG_QUEUE_NORMAL_ALLIANCE].begin();
                     itr != m_QueuedGroups[bracketId][BG_QUEUE_NORMAL_ALLIANCE].end(); ++itr)
                    if (!(*itr)->IsInvitedToBGInstanceGUID)
                        qAlliance += (*itr)->Players.size();
                for (itr = m_QueuedGroups[bracketId][BG_QUEUE_NORMAL_HORDE].begin();
                     itr != m_QueuedGroups[bracketId][BG_QUEUE_NORMAL_HORDE].end(); ++itr)
                    if (!(*itr)->IsInvitedToBGInstanceGUID)
                ...
                if (sWorld->getBoolConfig(CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_PLAYERONLY)) {
                    ChatHandler(leader->GetSession()).PSendSysMessage(LANG_BG_QUEUE_ANNOUNCE_SELF,
                                                                      bg->GetName().c_str(), q_min_level, q_max_level,
                                                                      qAlliance,
                                                                      (MinPlayers > qAlliance) ? MinPlayers - qAlliance
                                                                                               : (uint32) 0, qHorde,
                                                                      (MinPlayers > qHorde) ? MinPlayers - qHorde
                                                                                            : (uint32) 0);
                }
                    // System message
                else {
...
void
BattlegroundQueue::PlayerInvitedToBGUpdateAverageWaitTime(GroupQueueInfo *ginfo, BattlegroundBracketId bracket_id) {
...
void BattlegroundQueue::RemovePlayer(ObjectGuid guid, bool decreaseInvitedCount) {
    ...
    if (itr == m_QueuedPlayers.end()) {
        ...
        TC_LOG_DEBUG("bg.battleground", "BattlegroundQueue: couldn't find player %s (%s)", playerName.c_str(),
                     guid.ToString().c_str());
        return;
    }
    ...
    for (int32 bracket_id_tmp = MAX_BATTLEGROUND_BRACKETS - 1;
         bracket_id_tmp >= 0 && bracket_id == -1; --bracket_id_tmp) {
    ...
    if (bracket_id == -1) {
        TC_LOG_ERROR("bg.battleground", "BattlegroundQueue: ERROR Cannot find groupinfo for %s",
                     guid.ToString().c_str());
        return;
    }
    TC_LOG_DEBUG("bg.battleground", "BattlegroundQueue: Removing %s, from bracket_id %u", guid.ToString().c_str(),
                 (uint32) bracket_id);
    ...
    if (group->ArenaType && group->IsRated && group->Players.empty() &&
        sWorld->getBoolConfig(CONFIG_ARENA_QUEUE_ANNOUNCER_ENABLE))
        if (ArenaTeam *team = sArenaTeamMgr->GetArenaTeamById(group->ArenaTeamId))
            sWorld->SendWorldText(LANG_ARENA_QUEUE_ANNOUNCE_WORLD_EXIT, team->GetName().c_str(), group->ArenaType,
                                  group->ArenaType, group->ArenaTeamRating);
    ...
    if (group->IsInvitedToBGInstanceGUID && group->IsRated && decreaseInvitedCount) {
        if (ArenaTeam *at = sArenaTeamMgr->GetArenaTeamById(group->ArenaTeamId)) {
            TC_LOG_DEBUG("bg.battleground",
                         "UPDATING memberLost's personal arena rating for %s by opponents rating: %u",
                         guid.ToString().c_str(), group->OpponentsTeamRating);
...
bool BattlegroundQueue::InviteGroupToBG(GroupQueueInfo *ginfo, Battleground *bg, uint32 side) {
    ...
    if (!ginfo->IsInvitedToBGInstanceGUID) {
        ...
        for (std::map<ObjectGuid, PlayerQueueInfo *>::iterator itr = ginfo->Players.begin();
             itr != ginfo->Players.end(); ++itr) {
            // get the player
            ...
            BGQueueInviteEvent *inviteEvent = new BGQueueInviteEvent(player->GetGUID(),
                                                                     ginfo->IsInvitedToBGInstanceGUID, bgTypeId,
                                                                     ginfo->ArenaType, ginfo->RemoveInviteTime);
            m_events.AddEvent(inviteEvent, m_events.CalculateTime(Milliseconds(INVITATION_REMIND_TIME)));
            // create automatic remove events
            BGQueueRemoveEvent *removeEvent = new BGQueueRemoveEvent(player->GetGUID(),
                                                                     ginfo->IsInvitedToBGInstanceGUID, bgTypeId,
                                                                     bgQueueTypeId, ginfo->RemoveInviteTime);
            ...
            TC_LOG_DEBUG("bg.battleground",
                         "Battleground: invited player %s %s to BG instance %u queueindex %u bgtype %u",
                         player->GetName().c_str(), player->GetGUID().ToString().c_str(), bg->GetInstanceID(),
                         queueSlot, bg->GetTypeID());

            // send status packet
            sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bg, queueSlot, STATUS_WAIT_JOIN,
                                                            INVITE_ACCEPT_WAIT_TIME, 0, ginfo->ArenaType, 0);
...
void BattlegroundQueue::FillPlayersToBG(Battleground *bg, BattlegroundBracketId bracket_id) {
    ...
    */
    ...
    while (abs(diffAli - diffHorde) > 1 && (m_SelectionPools[TEAM_HORDE].GetPlayerCount() > 0 ||
        ...
        if (diffAli < diffHorde) {
            ...
            if (m_SelectionPools[TEAM_ALLIANCE].KickGroup(diffHorde - diffAli)) {
                for (; aliIndex < aliCount && m_SelectionPools[TEAM_ALLIANCE].AddGroup((*Ali_itr),
                                                                                       (aliFree >= diffHorde) ?
                                                                                       aliFree - diffHorde
                                                                                                              : 0); aliIndex++)
                    ++Ali_itr;
            ...
            if (m_SelectionPools[TEAM_HORDE].KickGroup(diffAli - diffHorde)) {
                for (; hordeIndex < hordeCount && m_SelectionPools[TEAM_HORDE].AddGroup((*Horde_itr),
                                                                                        (hordeFree >= diffAli) ?
                                                                                        hordeFree - diffAli
                                                                                                               : 0); hordeIndex++)
                    ++Horde_itr;
...
bool BattlegroundQueue::CheckPremadeMatch(BattlegroundBracketId bracket_id, uint32 MinPlayersPerTeam,
    ...
    if (!m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE].empty() &&
        ...
        for (ali_group = m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE].begin();
             ali_group != m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE].end(); ++ali_group)
            if (!(*ali_group)->IsInvitedToBGInstanceGUID)
                break;
        for (horde_group = m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE].begin();
             horde_group != m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE].end(); ++horde_group)
        ...
        if (ali_group != m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE].end() &&
            ...
            uint32 maxPlayers = std::min(m_SelectionPools[TEAM_ALLIANCE].GetPlayerCount(),
                                         m_SelectionPools[TEAM_HORDE].GetPlayerCount());
            GroupsQueueType::const_iterator itr;
            for (uint32 i = 0; i < PVP_TEAMS_COUNT; i++) {
                for (itr = m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE + i].begin();
                     itr != m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE + i].end(); ++itr) {
    ...
    uint32 time_before =
            GameTime::GetGameTimeMS() - sWorld->getIntConfig(CONFIG_BATTLEGROUND_PREMADE_GROUP_WAIT_FOR_MATCH);
    for (uint32 i = 0; i < PVP_TEAMS_COUNT; i++) {
...
bool BattlegroundQueue::CheckNormalMatch(Battleground *bg_template, BattlegroundBracketId bracket_id, uint32 minPlayers,
                                         uint32 maxPlayers) {
    ...
    if (sWorld->getIntConfig(CONFIG_BATTLEGROUND_INVITATION_TYPE) != BG_QUEUE_INVITATION_TYPE_NO_BALANCE
        ...
        for (; itr_team[j] != m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE + j].end(); ++(itr_team[j])) {
            if (!(*(itr_team[j]))->IsInvitedToBGInstanceGUID)
                if (!m_SelectionPools[j].AddGroup(*(itr_team[j]),
                                                  m_SelectionPools[(j + 1) % PVP_TEAMS_COUNT].GetPlayerCount()))
                    break;
        }
        // do not allow to start bg with more than 2 players more on 1 faction
        if (abs((int32) (m_SelectionPools[TEAM_HORDE].GetPlayerCount() -
...
bool BattlegroundQueue::CheckSkirmishForSameFaction(BattlegroundBracketId bracket_id, uint32 minPlayersPerTeam) {
    ...
    for (GroupsQueueType::iterator itr = m_SelectionPools[otherTeam].SelectedGroups.begin();
         itr != m_SelectionPools[otherTeam].SelectedGroups.end(); ++itr) {
        //set correct team
...
void BattlegroundQueue::BattlegroundQueueUpdate(uint32 /*diff*/, BattlegroundTypeId bgTypeId,
    ...
    for (BGFreeSlotQueueContainer::iterator itr = bgQueues.begin(); itr != bgQueues.end();) {
        ...
        if (!bg->isRated() && bg->GetTypeID() == bgTypeId && bg->GetBracketId() == bracket_id &&
            ...
            for (GroupsQueueType::const_iterator citr = m_SelectionPools[TEAM_ALLIANCE].SelectedGroups.begin();
                 citr != m_SelectionPools[TEAM_ALLIANCE].SelectedGroups.end(); ++citr)
                InviteGroupToBG((*citr), bg, (*citr)->Team);

            for (GroupsQueueType::const_iterator citr = m_SelectionPools[TEAM_HORDE].SelectedGroups.begin();
                 citr != m_SelectionPools[TEAM_HORDE].SelectedGroups.end(); ++citr)
                InviteGroupToBG((*citr), bg, (*citr)->Team);
    ...
    if (!bracketEntry) {
        TC_LOG_ERROR("bg.battleground", "Battleground: Update: bg bracket entry not found for map %u bracket id %u",
                     bg_template->GetMapId(), bracket_id);
        return;
    }
    ...
    if (bg_template->isBattleground()) {
        if (CheckPremadeMatch(bracket_id, MinPlayersPerTeam, MaxPlayersPerTeam)) {
            ...
            for (uint32 i = 0; i < PVP_TEAMS_COUNT; i++)
                for (GroupsQueueType::const_iterator citr = m_SelectionPools[TEAM_ALLIANCE + i].SelectedGroups.begin();
                     citr != m_SelectionPools[TEAM_ALLIANCE + i].SelectedGroups.end(); ++citr)
    ...
    if (!isRated) {
        ...
        //set rating range
        uint32 arenaMinRating = (arenaRating <= sBattlegroundMgr->GetMaxRatingDifference()) ? 0 : arenaRating -
                                                                                                  sBattlegroundMgr->GetMaxRatingDifference();
        ...
        int32 discardOpponentsTime =
                GameTime::GetGameTimeMS() - sWorld->getIntConfig(CONFIG_ARENA_PREV_OPPONENTS_DISCARD_TIMER);
            ...
            for (GroupsQueueType::iterator itr3 = itr_teams[0];
                ...
                if (!(*itr3)->IsInvitedToBGInstanceGUID
                    && (((*itr3)->ArenaMatchmakerRating >= arenaMinRating &&
                         (*itr3)->ArenaMatchmakerRating <= arenaMaxRating) || (int32) (*itr3)->JoinTime < discardTime)
        ...
        if (found == 2) {
            ...
            TC_LOG_DEBUG("bg.battleground", "setting oposite teamrating for team %u to %u", aTeam->ArenaTeamId,
                         aTeam->OpponentsTeamRating);
            TC_LOG_DEBUG("bg.battleground", "setting oposite teamrating for team %u to %u", hTeam->ArenaTeamId,
...
void BattlegroundQueue::CheckCustomArenaJoin() {
    ...
    for (int i = 0; i < betTeamVars.size(); i += 7) {
        ...
        // after teams are made we can generate arena battleground
        Battleground *bg_template_custom = sBattlegroundMgr->GetBattlegroundTemplate(bgTypeIdCustom);
        PvPDifficultyEntry const *bracketEntryCustom = GetBattlegroundBracketById(bg_template_custom->GetMapId(),
                                                                                  custom_bracket);
...
bool BGQueueInviteEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/) {
    ...
    if (queueSlot < PLAYER_MAX_BATTLEGROUND_QUEUES)         // player is in queue or in battleground
    {
        ...
        if (bgQueue.IsPlayerInvited(m_PlayerGuid, m_BgInstanceGUID, m_RemoveTime)) {
            ...
            sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, bg, queueSlot, STATUS_WAIT_JOIN,
                                                            INVITE_ACCEPT_WAIT_TIME - INVITATION_REMIND_TIME, 0,
                                                            m_ArenaType, 0);
            player->SendDirectMessage(&data);
        }
    }
...
bool BGQueueRemoveEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/) {
    ...
    if (queueSlot < PLAYER_MAX_BATTLEGROUND_QUEUES)         // player is in queue, or in Battleground
    {
        ...
        if (bgQueue.IsPlayerInvited(m_PlayerGuid, m_BgInstanceGUID, m_RemoveTime)) {
            ...
            TC_LOG_DEBUG("bg.battleground",
                         "Battleground: removing player %s from bg queue for instance %u because of not pressing enter battle in time.",
                         player->GetGUID().ToString().c_str(), m_BgInstanceGUID);
